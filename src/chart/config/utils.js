...
export function initHorizontalEchartGraphicShape() {
  ...
  const CubeLeftHor = echarts.graphic.extendShape({
    ...
    buildPath: function (ctx, shape) {
      const { yAxisPoint, xAxisPoint, offset, api, xValue } = shape;
      const xStarter = shape.xAxisPoint[0];
      const c0 = [shape.x, shape.y];
      const c1 = [xStarter, shape.y];
      const c2 = [xStarter, shape.y + (xValue ? longDiagonal : 0)];
      const c3 = [
        shape.x + (xValue ? shortDiagonal : 0),
        shape.y + (xValue ? longDiagonal : 0),
      ];
      ctx
        .moveTo(c0[0] + offset, c0[1])
        .lineTo(c1[0] + offset, c1[1])
  ...
  const CubeTopHor = echarts.graphic.extendShape({
    ...
    buildPath: function (ctx, shape) {
      const { offset, xValue } = shape;
      const shortDiagonals = xValue ? shortDiagonal : 0;
      const longDiagonals = xValue ? longDiagonal : 0;
      const c0 = [shape.x, shape.y];
      const c1 = [shape.x + shortDiagonals, shape.y + longDiagonals];
      const c2 = [shape.x + shortDiagonals * 2, shape.y];
      const c3 = [shape.x + shortDiagonals, shape.y - longDiagonals];
      ctx
        .moveTo(c0[0] + offset, c0[1])
        .lineTo(c1[0] + offset, c1[1])
        .lineTo(c2[0] + offset, c2[1])
